...
def import_10y_yields_data_series_from_fred(series_id, api_key=FRED_API_KEY):
    ...
    return data[["Date", "10y_Maturity_Bond_Yield"]]
...
def import_quarterly_data_from_OECD(xml_query_link, variable_name):
    ...
    check_for_duplicates(data[["Date", "REF_AREA"]])

    return data[["Date", variable_name, "REF_AREA"]]
...
def import_quarterly_gdp_data_in_USD_from_oecd(xml_query_link):
    ...
    return data[["Date", "REF_AREA", "GDP_in_USD_Current_Prices"]]
...
def import_quarterly_CPI_data_from_oecd(xml_query_link):
    ...
    # Correct datatypes
    data["Date"] = pd.to_datetime(data["TIME_PERIOD"])
    data["Eurostat_CPI_Annualised Growth_Rate"] = data[
        "Eurostat_CPI_Annualised Growth_Rate"
    ].astype(float)

    check_for_duplicates(
        data[["Date", "REF_AREA", "Eurostat_CPI_Annualised Growth_Rate"]],
    )

    return data[["Date", "REF_AREA", "Eurostat_CPI_Annualised Growth_Rate"]]
...
def import_real_quarterly_gdp_data_from_oecd(xml_query_link):
    ...
    # Correct datatypes
    data["Date"] = pd.to_datetime(data["TIME_PERIOD"])
    data["Real_Quarterly_GVA_in_Domestic_Currency"] = data[
        "Real_Quarterly_GVA_in_Domestic_Currency"
    ].astype(float)

    check_for_duplicates(data[["Date", "REF_AREA"]])

    return data[["Date", "REF_AREA", "Real_Quarterly_GVA_in_Domestic_Currency"]]
...
def import_quarterly_public_debt_as_percent_of_gdp_data_from_oecd(xml_query_link):
    ...
    check_for_duplicates(data[["Date", "Country"]])

    return data[["Date", "Country", "Public_Debt_as_%_of_GDP"]]
...
def import_quarterly_current_account_from_OECD(xml_query_link):
    ...
    check_for_duplicates(data[["Date", "Country", "Current_Account_in_USD"]])

    return data[["Date", "Country", "Current_Account_in_USD"]]


def clean_oecd_xml_response(data_as_dictionary):
    """This function returns a dataframe from the data received from OECD's API, which
    has to previously be converted into a dictionary.
    """
    full_data = pd.DataFrame()

    for i in range(
        len(
            data_as_dictionary["message:GenericData"]["message:DataSet"]["generic:Obs"],
        ),
    ):
        observation_dictionary = data_as_dictionary["message:GenericData"][
            "message:DataSet"
        ]["generic:Obs"][i]
        ...
        # ObsAttributes
        obs_attributes_data = observation_dictionary["generic:Attributes"][
            "generic:Value"
        ]
...
def check_for_duplicates(df: pd.DataFrame):
    """Raises an error if DataFrame has any duplicate rows."""
    if df.duplicated().any():
        msg = "DataFrame should not have any duplicate rows, but duplicates were found."
        raise ValueError(
            msg,
        )
